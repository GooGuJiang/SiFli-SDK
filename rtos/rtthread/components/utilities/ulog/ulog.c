...
struct rt_ulog
{
    ...
    rt_uint32_t loss_cnt;
    rt_uint32_t loss_bytes;
#endif
...
__ROM_USED size_t ulog_strcpy(size_t cur_len, char *dst, const char *src)
{
...
__ROM_USED size_t ulog_ultoa(char *s, unsigned long int n)
{
...
RT_WEAK rt_tick_t ulog_get_tick(void)
{
    return rt_tick_get();
}
#endif
                                ...
                                const char *format, va_list args)
{
    ...
    /* add time info */
    {
        ...
        log_buf[log_len] = '[';
        tick_len = ulog_ultoa(log_buf + log_len + 1, ulog_get_tick());
...
__ROM_USED void ulog_output_to_all_backend(rt_uint32_t level, const char *tag, rt_bool_t is_raw, const char *log, rt_size_t size)
{
    rt_slist_t *node;
...
static void do_output(rt_uint32_t level, const char *tag, rt_bool_t is_raw, const char *log_buf, rt_size_t log_len)
{
    ...
    if (log_blk)
    {
        ...
        /* send a notice */
#ifdef RT_USING_PM
        /* workaround as pendsv cannot be triggered if it's going to enter any low power mode */
        if (!ulog.suspended)
#endif  /* RT_USING_PM */
        {
 ...
 */
__ROM_USED void ulog_voutput(rt_uint32_t level, const char *tag, rt_bool_t newline, const char *format, va_list args)
{
    char *log_buf = NULL;
    rt_size_t log_len = 0;
 ...
 * @param ... args
 */
__ROM_USED void ulog_output(rt_uint32_t level, const char *tag, rt_bool_t newline, const char *format, ...)
{
    va_list args;
 ...
 * @param ... args
 */
__ROM_USED void ulog_raw(const char *format, ...)
{
    rt_size_t log_len = 0;
    char *log_buf = NULL;
    va_list args;
...
__ROM_USED void ulog_vraw(const char *format, va_list args)
{
    rt_size_t log_len = 0;
    char *log_buf = NULL;
    int fmt_result;
 ...
 */
__ROM_USED void ulog_hexdump(const char *tag, rt_size_t width, rt_uint8_t *buf, rt_size_t size)
{
#define __is_print(ch)       ((unsigned int)((ch) - ' ') < 127u - ' ')
    ...
    for (i = 0, log_len = 0; i < size; i += width)
    {
        ...
        RT_ASSERT(log_len < ULOG_LINE_BUF_SIZE);
        fmt_result = rt_snprintf(log_buf + log_len, ULOG_LINE_BUF_SIZE - log_len, "%04X-%04X: ", i, i + width);
        /* calculate log length */
        if ((fmt_result > -1) && (fmt_result <= (int)(ULOG_LINE_BUF_SIZE - log_len)))
        {
            log_len += fmt_result;
        }
        else
...
{
    output_lock();
    int type = is_mix ? RAW_BIN_MIX : RAW_BIN;

    do_output(LOG_LVL_ERROR, NULL, type, (const char *)buf, size);
 ...
 */
__ROM_USED int ulog_tag_lvl_filter_set(const char *tag, rt_uint32_t level)
{
    rt_slist_t *node;
 ...
 */
__ROM_USED rt_uint32_t ulog_tag_lvl_filter_get(const char *tag)
{
    rt_slist_t *node;
    ulog_tag_lvl_filter_t tag_lvl = NULL;
 ...
 */
__ROM_USED rt_slist_t *ulog_tag_lvl_list_get(void)
{
 ...
 */
__ROM_USED void ulog_global_filter_lvl_set(rt_uint32_t level)
{
 ...
 *              LOG_FILTER_LVL_ALL: enable all log output
 */
__ROM_USED rt_uint32_t ulog_global_filter_lvl_get(void)
{
 ...
 * @param tag tag
 */
__ROM_USED void ulog_global_filter_tag_set(const char *tag)
{
    RT_ASSERT(tag);
 ...
 * @return tag
 */
__ROM_USED const char *ulog_global_filter_tag_get(void)
{
 ...
 */
__ROM_USED void ulog_global_filter_kw_set(const char *keyword)
{
    RT_ASSERT(keyword);
...
__ROM_USED void ulog_tag_lvl(uint8_t argc, char **argv)
{
    if (argc > 2)
    {
...
__ROM_USED void ulog_lvl(uint8_t argc, char **argv)
{
    if (argc > 1)
    {
...
__ROM_USED void ulog_tag(uint8_t argc, char **argv)
{
    if (argc > 1)
    {
...
__ROM_USED void ulog_kw(uint8_t argc, char **argv)
{
    if (argc > 1)
    {
...
__ROM_USED void ulog_filter(uint8_t argc, char **argv)
{
#ifndef ULOG_USING_SYSLOG
...
__ROM_USED rt_err_t ulog_backend_register(ulog_backend_t backend, const char *name, rt_bool_t support_color)
{
    rt_base_t level;
...
__ROM_USED rt_err_t ulog_backend_unregister(ulog_backend_t backend)
{
    rt_base_t level;
 ...
 */
__ROM_USED void ulog_async_output(void)
{
    rt_rbb_blk_t log_blk;
    ulog_frame_t log_frame;
    ...
    if (ulog.loss_cnt > 0)
    {
        log_blk = rt_rbb_blk_alloc(ulog.async_rbb, RT_ALIGN(sizeof(struct ulog_frame) + ULOG_LINE_BUF_SIZE, RT_ALIGN_SIZE));
        if (log_blk)
        {
            rt_uint32_t loss_cnt;
            rt_uint32_t loss_bytes;
            ...
            output_lock();
            loss_cnt = ulog.loss_cnt;
            loss_bytes = ulog.loss_bytes;
            ulog.loss_cnt = 0;
            ulog.loss_bytes = 0;
            output_unlock();
            ...
            log_frame = (ulog_frame_t) log_blk->buf;
            log_frame->magic = ULOG_FRAME_MAGIC;
            log_frame->is_raw = RT_TRUE;
            log_frame->level = LOG_LVL_DBG;
            log_frame->tag = RT_NULL;
            log_frame->log = (const char *)log_blk->buf + sizeof(struct ulog_frame);
            /* copy log data */
            log_frame->log_len = rt_snprintf((char *)log_frame->log, ULOG_LINE_BUF_SIZE, "\nULOG_WARN: trace loss %d,%d\n", loss_cnt, loss_bytes);
            RT_ASSERT(log_frame->log_len > 0);
            RT_ASSERT(log_frame->log_len <= ULOG_LINE_BUF_SIZE);
            /* put the block */
            rt_rbb_blk_put(log_blk);

            if ((log_blk = rt_rbb_blk_get(ulog.async_rbb)) != NULL)
            {
                log_frame = (ulog_frame_t) log_blk->buf;
                if (log_frame->magic == ULOG_FRAME_MAGIC)
                {
 ...
 */
__ROM_USED void ulog_async_waiting_log(rt_int32_t time)
{
 ...
 */
__ROM_USED void ulog_flush(void)
{
    rt_slist_t *node;
    ulog_backend_t backend;
...
__ROM_USED void ulog_deinit(void)
{
    rt_slist_t *node;
    ulog_backend_t backend;
...
__ROM_USED rt_bool_t ulog_init_ok(void)
{
    return ulog.init_ok;
}

void ulog_pause(int pause)
{
    static rt_bool_t status = RT_FALSE;
    if (pause && ulog.init_ok)
        status = RT_TRUE;
    ulog.init_ok = pause ? RT_FALSE : status;
}

#if defined(RT_USING_PM) && defined(ULOG_USING_ASYNC_OUTPUT)

static int ulog_pm_suspend(const struct rt_device *device, uint8_t mode)
{
    ulog.suspended = RT_TRUE;

    return RT_EOK;

}

void ulog_pm_resume(const struct rt_device *device, uint8_t mode)
{
    ulog.suspended = RT_FALSE;

    return;
}

const struct rt_device_pm_ops ulog_pm_op =
{
    .suspend = ulog_pm_suspend,
    .resume = ulog_pm_resume,
};


int ulog_pm_init(void)
{
    rt_pm_device_register(NULL, &ulog_pm_op);

    return 0;

}
INIT_ENV_EXPORT(ulog_pm_init);
