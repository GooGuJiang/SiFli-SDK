...
{
    uint32_t value = READ_REG(hwp_audcodec->PLL_CFG3);
    int32_t sdin;


    sdin = (int32_t)(value & AUDCODEC_PLL_CFG3_SDIN_Msk) + (int32_t)delta;
...
// target difference between pll_cnt and xtal_cnt should be less than 1
        ...
        if (delta_cnt_min <= delta_cnt && delta_cnt_min <= delta_cnt_max)
        {
            ...
            hwp_audcodec->PLL_CFG0 |= ((fc_vco_min - 2) << AUDCODEC_PLL_CFG0_FC_VCO_Pos);
        }
        else if (delta_cnt_max <= delta_cnt && delta_cnt_max <= delta_cnt_min)
        {
            fc_vco_max  = (fc_vco_max  < 2) ? 2 : fc_vco_max;
            hwp_audcodec->PLL_CFG0 |= ((fc_vco_max - 2) << AUDCODEC_PLL_CFG0_FC_VCO_Pos);
        }
        else
        {
            fc_vco  = (fc_vco  < 2) ? 2 : fc_vco;
            hwp_audcodec->PLL_CFG0 |= ((fc_vco - 2) << AUDCODEC_PLL_CFG0_FC_VCO_Pos);
        }
#else
        if (delta_cnt_min <= delta_cnt && delta_cnt_min <= delta_cnt_max)
        {
...
{
    volatile uint32_t *ptr = (volatile uint32_t *)AUDCODEC_BASE; //LP_BASE
    int i;
    for (i = 0; i < 40; i++)
    {
        LOG_RAW("0x%08x ", *ptr);
...
static rt_err_t bf0_audio_configure(struct rt_audio_device *audio, struct rt_audio_caps *caps)
{
    ...
    case AUDIO_TYPE_INPUT:
    {
        ...
        if (caps->sub_type & (1 << (HAL_AUDCODEC_ADC_CH1)))
        {
            ...
            {
                haudcodec->buf[HAL_AUDCODEC_ADC_CH1] = dma_malloc_in_1m(1, haudcodec->bufSize);
                if (haudcodec->buf[HAL_AUDCODEC_ADC_CH1] == NULL)
                    return RT_ERROR_MEMFAULT;
            }
#endif
...
#ifdef AUDCODEC_DAC1_DMA_INSTANCE
#ifndef DMA_SUPPORT_DYN_CHANNEL_ALLOC
            HAL_NVIC_DisableIRQ(AUDCO - DEC_DAC1_DMA_IRQ);
#endif /* !DMA_SUPPORT_DYN_CHANNEL_ALLOC */
